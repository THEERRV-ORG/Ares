...
const CpuArchitecture = ({
  ...
  // Fit long labels into at most 2 lines: first word alone, remaining words together.
  const lines = words.length > 1 ? [words[0], words.slice(1).join(" ")] : words;
  const longestLine = Math.max(...lines.map((l) => l.length));
  const fontSize = longestLine <= 4 ? 8 : longestLine <= 6 ? 6 : 4.5;
  ...
  return (
    ...
    >
      ...
      <g>
        ...
        >
          {lines.map((line, i) => (
            <tspan key={line} x="100" y={startY + i * lineHeight}>
              {line}
            </tspan>
          ))}
        </text>
